  ...
  return (
    <div className="space-y-16">
      {blocks?.map((block, index) => {
        switch (block._type) {
          case "heroImage":
  const imageUrl = block.image?.asset?.url || urlFor(block.image).url();
  return (
    <section key={index} className="relative text-center mb-4">  {/* reduced bottom margin */}
      {block.image && (
        <Image
          src={imageUrl}
          alt={block.text || "Hero Image"}
          width={1200}
          height={300}   {/* thinner height */}
      ...
      {block.text && (
        ...
        >
          <h2 className="text-3xl font-bold text-white bg-black/50 p-4 rounded">
            {block.text}
          </h2>
        </div>
      )}
